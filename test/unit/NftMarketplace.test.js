...
!developmentChains.includes(network.name)
  ...
  : describe("Nft Marketplace Tests", async () => {
      ...
      describe("List Item", async () => {
        ...
        it("exclusively items owners to list", async () => {
          nftMarketplace = nftMarketplaceContract.connect(user)
          await basicNft.approve(basicNft.address, TOKEN_ID)
          await expect(
            nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
        ...
        it("revert if price 0", async () => {
          const ZERO_PRICE = await ethers.utils.parseEther("0")
          await expect(
            nftMarketplace.listItem(basicNft.address, TOKEN_ID, ZERO_PRICE)
          ).to.be.revertedWith("NftMarketplace__PriceMustBeAbouveZero")
        })
        ...
        it("Update listing with price and seller", async () => {
          await nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
      ...
      describe("Cancel Listing", async () => {
        ...
        it("emit event and remove listing", async () => {
          await nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
      ...
      describe("witdraw proceeds ", async () => {
